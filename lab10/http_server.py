...
CUR = CONN.cursor()
CUR.execute('''CREATE TABLE IF NOT EXISTS Log (id INTEGER PRIMARY KEY AUTOINCREMENT, ip TEXT, request TEXT);''')
CONN.commit()
...
class MyHTTPRequestHandler(http.server.BaseHTTPRequestHandler): 
    ...
    def do_GET(self): 
        ...
        self.cur.execute('''INSERT INTO Log (ip, request) VALUES (?, ?);''', (self.client_address[0], self.requestline))
        self.conn.commit()
...
def main():
    ...
    with MyHTTPDaemon((FLAGS.host, FLAGS.port), MyHTTPRequestHandler) as httpd:
        try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--debug', action='store_true',help='Debug message')
    ...
    parser.add_argument('--rootdir', default='./dir', type=str, help='Root directory')
